    ...
    <div>
      {!isFullScreen && (
        <Sidebar
          collapsible="icon"
          {...props}
          className={`bg-[var(--background)] dark:bg-darkBackground flex h-screen select-none flex-col items-center ${state === 'collapsed' ? '' : ''} pb-2`}
        >
          <SidebarHeader
          ...
          <SidebarContent
            className={`scrollbar scrollbar-w-1 scrollbar-thumb-primary/40 scrollbar-track-transparent hover:scrollbar-thumb-primary scrollbar-thumb-rounded-full overflow-x-hidden py-0 pt-0 ${state !== 'collapsed' ? 'mt-5 md:px-4' : 'px-2'}`}
          >
            <AnimatePresence mode="wait">
              <motion.div
                key={currentSection}
  ...
  return (
    <Dialog open={!!dialogOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogTrigger asChild>
        <button className="inline-flex h-12 w-full items-center justify-center gap-1 self-stretch overflow-hidden rounded-xl bg-gradient-to-r from-primary to-primary/65 text-white dark:border-none dark:bg-gradient-to-b dark:from-white/20 dark:to-white/10 dark:text-white dark:outline dark:outline-1 dark:outline-offset-[-1px] dark:outline-white/5">
          {state === 'collapsed' && !isMobile ? (
            <PencilCompose className="fill-white dark:fill-iconDark mt-0.5 text-black" />
          ) : (
            <div className="flex items-center justify-center gap-2.5 pl-0.5 pr-1">
              <PencilCompose className="fill-white dark:fill-iconDark" />
